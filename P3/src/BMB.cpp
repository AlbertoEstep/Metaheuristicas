...
void busquedaMultiarranqueBasica(vector<vector<double>> &m, int n, const int MAX_EVALUACIONES){
  ...
  for(int i = 0; i < iteraciones; ++i){
    solucionAleatoria(m, s, n);
    evaluarSolucion(s, m);
    evaluaciones_iteracion = busquedaLocal(m, s, MAX_EVALUACIONES);
    evaluaciones += 1 + evaluaciones_iteracion;
    // cout << "Iteración " << i << ": evaluaciones: " << evaluaciones_iteracion << endl;
    if(s.diversidad > mejor_diversidad)
      mejor_diversidad = s.diversidad;
  }
...
int main(int argc, char *argv[]){
  ...
  busquedaMultiarranqueBasica(m, n_sel, MAX_EVALUACIONES);
}
